...
func singleUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	// /api/users/1 разбивается на 4 части: ["", "api", "users", "1"]
	if len(pathParts) < 4 {
	...
	// Добавлено разделение логики по HTTP-методам
	...
	if r.Method == http.MethodDelete {
		...
		// Раскомментирована и исправлена обработка ошибки
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
	...
	// Если метод не GET и не DELETE
	http.Error(w, "Method not allowed", http.StatusMethodNotAllowed)
}

func homeHandler(w http.ResponseWriter, r *http.Request) {
	// Убран слеш в начале, чтобы путь искался от папки запуска программы
